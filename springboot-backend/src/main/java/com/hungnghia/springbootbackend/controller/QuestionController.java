...
import com.hungnghia.springbootbackend.dto.QuestionListenAddDto;
...
public class QuestionController {
    ...
    @PostMapping("/addQuestionListen")
    public ResponseEntity<QuestionEntity> addQuestion(@RequestPart("QuestionListenAddDto") QuestionListenAddDto questionListenAddDto, @RequestPart(value = "fileImage", required = false) MultipartFile fileImage, @RequestPart("fileAudio") MultipartFile fileAudio) {
        QuestionEntity result = questionService.addQuestionListen(questionListenAddDto, fileImage, fileAudio);
        if (result != null) {
            return ResponseEntity.ok(result);
        } else {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
